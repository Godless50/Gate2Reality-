...
**Статус:** ✅ реализовано (L1+L2+L3). Файлы в `Gate2Reality.Persistence`:
`AnchorRecord`/`RoomFingerprint`, `IAnchorRegistry`/`AnchorRegistry`,
`AnchorSerializer`, `OfflineAnchorRelocalizer`, `RelocalizationResult`.
Финальные пороги (окно L2, допуск fingerprint) — поля инспектора
`OfflineAnchorRelocalizer`, калибруются на Stage A (Pixel 9 / HONOR 90).
...
**Решение (реализовано):** L1+L2+L3 целиком. Автор подтвердил, что YOLO
отлично работает на HONOR 90 (mid-tier эталон) и сеть не нужна — значит
дорогого окна детекции бояться нечего, а точность L2 критична для портала
Главы II в стене. L2 включён по умолчанию (`enableL2 = true`); окно
(`l2WindowSeconds = 3`) с ранним выходом при нахождении всех меток и допуск
fingerprint (`l2FingerprintTolerance = 0.35 м`) вынесены в инспектор —
финальная калибровка на Stage A, как в `DeviceTuningProfile`.
...
## 12. Решённые вопросы
1. **Опорный объект комнаты** — Chair по умолчанию
   (`ProgressTracker.referenceAnchorLabel`), но `AnchorSerializer.Capture`
   автоматически падает на первый валидный якорь, если Chair не зарегистрирован
   (L2′ из §7). Стул — самый «тяжёлый»/стабильный объект по ТЗ.
2. **L2 в v1** — да, включён (см. §9): YOLO подтверждён на HONOR 90.
3. **Дрейф L3** — числового порога из ТЗ нет; L3 сознательно «неприклеенный»
   фолбэк, портал Главы II при L3 не гарантирует попадание в физическую стену
   (guard доведёт). Точность для портала обеспечивает L2. Замер дрейфа L3
   рулеткой у стены — пункт Stage A (§10).

---

## 13. Stage A телеметрия
`OfflineAnchorRelocalizer.OnRelocalizationReported(level, count)` — статическое
событие; отладочный HUD (`DetectionDebugHud`, dev-only) показывает строку
`Reloc: L{1/2/3} N anchor(s)` с цветовым кодом (L1 зелёный / L2 циан / L3
жёлтый). Это даёт на полевом прогоне мгновенный ответ, каким путём поднялись
якоря после холодного старта.
