...
#pragma warning restore CS0414
        ...
        private static string BuildPromptV2(
            string subjectRu,
            string contextChain,
            NarrativeManager.NarrativeStage stage,
            in NarrativeContext ctx)
        {
            s_Prompt.Clear();
            s_Prompt.Append("Ты — барабашка. Тебе несколько сотен лет. Ты маленький, мохнатый, невидимый когда захочешь. Живёшь в этом пространстве и помнишь всё. Говори коротко — 1-2 предложения. Иногда обрывай фразу. Запрещено: призрак, дух, смерть, убийство. Говори от первого лица.\n\n");
            s_Prompt.Append("Объект: ");
            s_Prompt.Append(subjectRu);
            s_Prompt.Append(". ");
            if (!string.IsNullOrEmpty(contextChain))
            {
                s_Prompt.Append("Предыдущие объекты: ");
                s_Prompt.Append(contextChain);
                s_Prompt.Append(". ");
            }
            s_Prompt.Append("Стадия: ");
            s_Prompt.Append(StageToRussian(stage));
            s_Prompt.Append(". ");
            s_Prompt.Append(ctx.AmbientBrightness < 0.35f ? "Освещение: тёмно. " : "Освещение: нормально. ");
            s_Prompt.Append("Напиши 1-2 предложения от первого лица.");
            return s_Prompt.ToString();
        }

        private static string StageToRussian(NarrativeManager.NarrativeStage s) => s switch
        {
            NarrativeManager.NarrativeStage.Prologue   => "завязка",
            NarrativeManager.NarrativeStage.Rising     => "развитие",
            NarrativeManager.NarrativeStage.Escalation => "нагнетание",
            _                                          => "кризис"
        };
