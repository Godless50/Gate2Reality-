...
**Расхождение, найденное тестами (закрыто опцией):** `_idleTimer` описан как
«секунды с последнего прогресса», но по умолчанию guard считает «время в узле».
Добавлен тоггл `resetIdleOnProgress` (по умолчанию **OFF** — ноль изменений к
as-tested сборке; **ON** включает поведение из комментария). Оба пути покрыты
play-mode тестами `Guard_DefaultOff_FiresDespiteProgress` /
`Guard_ResetOnProgress_StaysDormantWhileActive`.
